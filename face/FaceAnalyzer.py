...
def resize_rect(rect, upsample):
    ...
    return (np.asarray(tblr) * upsample).tolist()

def resize_shape(shape, upsample):
    parts = [(e.x, e.y) for e in shape.parts()]
    return (np.asarray(parts) * upsample).tolist()
...
class FaceAnalyzer:
    def __init__(self):
        ...
        self.expression_classifier = ort.InferenceSession('models/affectnet.onnx')
        with open('models/affectnet_classes.txt') as f:
    ...
    def __call__(self, img, downsample=2):
        ...
        descriptors = [np.asarray(e).tolist() for e in descriptors]
        expressions = [dict(zip(self.expression_classes, softmax(e).reshape(-1))) for e in expressions]
        return list(zip(rects, shapes, descriptors, expressions))